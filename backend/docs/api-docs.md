...
| `from_user_id` | string | Khong | Không còn khuyến nghị trong family mode; người trả nợ được suy ra từ người dùng đang đăng nhập |
...
| `family_id` | string | Khong | UUID gia đình dùng khi tất toán khoản chia sẻ chi phí giữa các thành viên |
...
Dự báo xu hướng dòng tiền dựa trên lịch sử giao dịch của người dùng.

### GET `/api/forecast`

**Lấy dữ liệu dự báo dòng tiền tiêu chuẩn**

Trả về tổng thu/chi theo tháng trong quá khứ cùng dữ liệu dự báo cho các tháng tiếp theo.
...
| Tham so | Vi tri | Mo ta | Bat buoc | Kieu du lieu |
| --- | --- | --- | --- | --- |
| `months` | query | Số tháng tương lai cần dự báo | Khong | integer [mac dinh: 3] |
...
**Lấy dữ liệu dự báo AI/ML**

Sử dụng hồi quy tuyến tính đơn giản trên lịch sử giao dịch 6 tháng gần nhất.
...
Dữ liệu thị trường và các tiện ích tích hợp cho dashboard của người dùng đã đăng nhập.

### GET `/api/market/gold`

**Lấy giá vàng SJC mới nhất cho dashboard**

Gọi dịch vụ giá vàng SJC trực tiếp, chuẩn hóa bản ghi SJC khu vực TP. Hồ Chí Minh và lưu cache Redis trong 60 giây.
...
**Lấy lịch sử giá vàng SJC cho biểu đồ dashboard**

Trả về lịch sử giá vàng SJC đang lưu cục bộ theo khoảng thời gian đã chọn. Dữ liệu trực tiếp được ưu tiên hơn dữ liệu demo nếu trùng thời điểm.
